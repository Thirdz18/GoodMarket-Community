...
- `SUPPORT_DESK_BASE_URL` — Base URL for the GIMT Support Desk backend that receives GoodMarket ticket creation requests.
- `GOODMARKET_SUPPORT_API_SECRET` — Shared bearer-token secret used only by the GoodMarket server when calling the support desk.



## Support Desk Integration

GoodMarket includes a `/support` page and a server-side `POST /api/support-tickets` endpoint for creating support tickets in the external GIMT Support Desk system. The browser submits only to GoodMarket; GoodMarket then forwards the request to the configured support-desk backend using a server-side bearer token.

Configure these GoodMarket environment variables:
...
```http
POST /api/goodmarket/support-tickets
Authorization: Bearer ${GOODMARKET_SUPPORT_API_SECRET}
Content-Type: application/json
```

GoodMarket forwards a payload with `source: "goodmarket"`, `externalProject: "goodmarket"`, GoodMarket user/contact fields, optional order ID, category, message, and optional attachment URL. The support desk should create records in its existing `support_tickets`, `support_ticket_messages`, and `support_ticket_events` tables and return JSON similar to:

```json
{
  "ok": true,
  "ticketCode": "SUP-20260515-ABCD",
  "status": "open",
  "message": "Your support ticket has been created."
}
```

If either GoodMarket env var is missing, or the support-desk endpoint is unavailable, GoodMarket keeps the secret server-side and shows a clear failure message instead of faking ticket creation.
