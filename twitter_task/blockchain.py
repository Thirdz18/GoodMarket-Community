...
class TwitterTaskBlockchain:
    ...
    def _disburse_via_fallback_key(self, wallet_address: str, reward_amount_wei: int, task_id: str) -> dict:
        ...
        logger.warning(f"🚨 FALLBACK TRIGGERED [twitter] — using DAILYTASK_KEY {masked} to direct-transfer G$ to user (task_id={task_id})")
        ...
        if receipt.status == 1:
            logger.warning(
                f"✅ FALLBACK SUCCESS [twitter] — DAILYTASK_KEY paid out "
