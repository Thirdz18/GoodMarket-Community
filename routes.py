...
SUPPORT_TICKET_CATEGORY_SET = set(SUPPORT_TICKET_CATEGORIES)
...
def _clean_support_string(value, max_length):
    """Normalize support-ticket string inputs and enforce length caps."""
    if value is None:
        return ""
...
def _is_valid_support_url(value):
    """Allow optional http(s) attachment URLs only."""
    if not value:
        return True
    try:
...
def _support_session_user(payload):
    ...
    return {
        ...
        "name": user_name,
        "email": user_email,
        "is_authenticated": bool(wallet and verified),
    }
...
def support_page():
    """Support request form that forwards tickets to the configured support desk."""
    ...
    return render_template(
        ...
        prefill_order_id=(request.args.get("orderId") or request.args.get("order_id") or "")[:SUPPORT_FIELD_LIMITS["orderId"]],
    )


@routes.route("/api/support-tickets", methods=["POST"])
def create_support_ticket():
    """Validate a GoodMarket support request and proxy it to GIMT Support Desk."""
    payload = request.get_json(silent=True) or {}
    if not isinstance(payload, dict):
        return jsonify({"success": False, "error": "Invalid JSON body"}), 400

    try:
        category = _clean_support_string(payload.get("category"), SUPPORT_FIELD_LIMITS["category"])
        order_id = _clean_support_string(payload.get("orderId"), SUPPORT_FIELD_LIMITS["orderId"])
        message = _clean_support_string(payload.get("message"), SUPPORT_FIELD_LIMITS["message"])
        attachment_url = _clean_support_string(payload.get("attachmentUrl"), SUPPORT_FIELD_LIMITS["attachmentUrl"])
        user = _support_session_user(payload)
    except ValueError as exc:
        return jsonify({"success": False, "error": str(exc)}), 400

    if not category:
        return jsonify({"success": False, "error": "Category is required"}), 400
    if category not in SUPPORT_TICKET_CATEGORY_SET:
        return jsonify({"success": False, "error": "Choose a valid support category"}), 400
    if not message:
        return jsonify({"success": False, "error": "Message is required"}), 400
    if not _is_valid_support_url(attachment_url):
        return jsonify({"success": False, "error": "Attachment URL must be a valid http(s) URL"}), 400

    # GoodMarket supports public browsing, so unauthenticated support requests are
    # accepted only when the user provides enough contact details for follow-up.
    if not user["is_authenticated"]:
        if not user["name"]:
            return jsonify({"success": False, "error": "Name is required for guest support requests"}), 400
        if not user["email"] or "@" not in user["email"]:
            return jsonify({"success": False, "error": "A valid email is required for guest support requests"}), 400
    ...
    outbound_request = urllib.request.Request(
        ...
        method="POST",
        headers={
            "Authorization": f"Bearer {support_secret}",
            "Content-Type": "application/json",
            "Accept": "application/json",
            "User-Agent": "GoodMarket-Support-Integration/1.0",
        },
    )

    try:
        with urllib.request.urlopen(outbound_request, timeout=12) as response:
    ...
    return jsonify({
        ...
        "status": desk_response.get("status", "open"),
        "message": f"Your support ticket #{ticket_code} has been created.",
    })
