...
@routes.route("/api/admin/goodmarket-verified-users", methods=["GET"])
@admin_required
def admin_goodmarket_verified_users():
    """Admin: list users whose face verification is attributed to GoodMarket."""
    try:
        ...
        users = safe_supabase_operation(
            lambda: supabase.table("user_data")
                .select(
                    "wallet_address, username, ubi_verified, face_verified, "
                    "verified_after_goodmarket, first_seen_unverified, "
                    "verification_timestamp, face_verified_at, total_logins, "
            ...
            operation_name="get GoodMarket verified users",
        )
        ...
        logger.error(f"❌ Get GoodMarket verified users error: {e}")
        return jsonify({"success": False, "error": str(e)}), 500


@routes.route("/api/admin/goodmarket-history", methods=["GET"])
@admin_required
def admin_goodmarket_history():
    """Admin: latest GoodMarket claim/verification event history, capped at 100 rows."""
    try:
        supabase = get_supabase_admin_client() or get_supabase_client()
        if not supabase:
            return jsonify({"success": False, "error": "Database not available"}), 500
        ...
        events = safe_supabase_operation(
            lambda: supabase.table("goodmarket_claim_events")
                .select(
                    "id, claim_attempt_id, wallet_address, network, tx_hash, "
                    "event_type, source, correlation_id, error_code, "
                ...
                .execute(),
            fallback_result=type("obj", (object,), {"data": []})(),
            operation_name="get GoodMarket event history",
        )

        rows = events.data if events.data else []
        ...
        logger.error(f"❌ Get GoodMarket history error: {e}")
        return jsonify({"success": False, "error": str(e)}), 500
