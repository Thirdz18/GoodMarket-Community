...
def turnkey_create_wallet():
    ...
    try:
        ...
        # Create a self-custody compatible key first, then import that key into Turnkey.
        # This preserves Turnkey signing while allowing explicit private-key export later.
        acct = Account.create()
        ...
        wallet_address = result.get("address") or acct.address
...
# ── Gas Faucet ──────────────────────────────────────────────────────────────
# In-memory rate-limit store: wallet_address -> unix timestamp of last top-up
...
def gas_faucet():
    ...
    try:
        ...
        celo_bal = float(w3.from_wei(celo_wei, "ether"))

        if celo_bal >= FAUCET_MIN_CELO:
            return jsonify({"success": True, "topped_up": False,
                            "reason": f"Balance OK ({celo_bal:.4f} CELO)", "balance": celo_bal})

        # Rate-limit check
        now = time.time()
        with _faucet_lock:
            last = _faucet_last_sent.get(checksum_wallet.lower(), 0)
            if now - last < FAUCET_COOLDOWN_HOURS * 3600:
                remaining_h = (FAUCET_COOLDOWN_HOURS * 3600 - (now - last)) / 3600
                return jsonify({"success": True, "topped_up": False,
...
def export_private_key():
    """Return the decrypted private key for users with an exportable key in session."""
    if session.get("login_method") not in ("custodial", "turnkey"):
        return jsonify({"success": False, "error": "Only available for custodial or turnkey wallets"}), 403
    enc_key = session.get("custodial_key_enc")
    if not enc_key:
        return jsonify({"success": False, "error": "No exportable key in session — please log in again"}), 401
    try:
        fernet = _get_fernet()
        private_key = fernet.decrypt(enc_key.encode()).decode()
